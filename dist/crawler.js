...
const puppeteer_1 = __importDefault(require("puppeteer")); // Only Browser and Page are directly imported
const commander_1 = require("commander");
const fs_1 = require("fs");
const url_1 = require("url");
const path_1 = __importDefault(require("path"));
const robots_txt_parser_1 = __importDefault(require("robots-txt-parser")); // <-- این خط را تغییر دهیدimport * as xml2js from 'xml2js';
...
const normalizeUrl = (url) => {
    try {
        ...
        console.warn(`[WARN] Failed to normalize URL "${url}": ${e.message}`);
...
class WebsiteCrawler {
    ...
    constructor(config) {
        ...
        this.robots = null;
        this.sitemapUrls = new Set(); // Use Set to avoid duplicate sitemap URLs
        this.activePages = 0; // Counter for currently active browser pages
        this.config = config;
    ...
    async parseRobotsTxt() {
        const robotsUrl = new url_1.URL('/robots.txt', this.config.startUrl).toString();
        let page = null;
        try {
            ...
            if (response && response.status() === 200) {
                ...
                this.robots = (0, robots_txt_parser_1.default)(robotsUrl, text); // <-- CHANGE THIS LINE
            ...
            console.warn(`[ERROR] Error parsing robots.txt from ${robotsUrl}: ${error.message}`);
        }
        finally {
            if (page) {
                await page.close();
    ...
    async fetchSitemap(url) {
        ...
        try {
            ...
            if (parsed.sitemapindex && parsed.sitemapindex.sitemap) {
                ...
                urls = urlsInSet
                    .map((entry) => entry.loc)
                    .filter((loc) => typeof loc === 'string'); // Filter out invalid URLs
            }
            console.log(`[INFO] Extracted ${urls.length} URLs from sitemap: ${url}`);
            return urls;
        }
        catch (error) {
            console.warn(`[ERROR] Error fetching or parsing sitemap from ${url}: ${error.message}`);
            return [];
        }
        finally {
            if (page) {
     ...
     */
    async crawlPage(url, depth, queue) {
        let page = null;
        const normalizedUrl = normalizeUrl(url);
        const node = {
            url,
        ...
        const discoveredJsUrlsForThisPage = new Set(); // Use Set for unique JS links per page
        try {
            const startTime = perf_hooks_1.performance.now();
            page = await this.browser.newPage();
            ...
            node.loadTime = perf_hooks_1.performance.now() - startTime;
            node.title = await page.title();
            // Collect Security/Reconnaissance data if safeClick is enabled
            if (this.config.safeClick) {
                // Detect forms
                node.forms = await page.$$eval('form', (forms) => forms.map((form) => form.action || form.id || 'unnamed_form_[' + Math.random().toString(36).substring(7) + ']'));
                // Capture cookies
            ...
            for (const link of extractedLinks) {
                ...
                const baseUrl = new url_1.URL(this.config.startUrl); // Reference domain for internal links
            ...
            node.errors.push(`[CRAWL_ERROR] ${error.message}`);
            ...
            console.warn(`[WARN] Failed to crawl ${url}: ${error.message}`);
        }
        finally {
            if (page) {
                await page.close();
    ...
    async extractLinks(page) {
        try {
            const links = await page.$$eval('a[href], link[rel="alternate"][href], area[href]', (elements, pageUrl) => Array.from(elements)
                .map((el) => el.getAttribute('href'))
                .filter((href) => !!href && href.trim() !== '') // Filter out empty/null hrefs
                .map((href) => {
                try {
                    // Construct absolute URL from href, relative to pageUrl
                    return new url_1.URL(href, pageUrl).toString();
                }
                catch {
            ...
            console.warn(`[WARN] Failed to extract links from ${page.url()}: ${error.message}`);
            return [];
        }
    }
    /**
    ...
    async handleInfiniteScroll(page) {
        try {
            ...
            console.warn(`[WARN] Infinite scroll failed on ${page.url()}: ${error.message}`);
        }
    }
    /**
     ...
     * @param depth - Current crawl depth.
     */
    async simulateSafeClicks(page, sourceUrl, queue, depth) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const clickableElementsInfo = await page.evaluate((selector, kws) => {
                const uniqueElements = []; // Moved initialization here
            ...
            for (const elementInfo of clickableElementsInfo) {
                try {
                    ...
                    const targetElementHandle = await page.evaluateHandle((info, selector) => {
                        const elements = document.querySelectorAll(selector);
                        for (const el of Array.from(elements)) { // Cast Array.from result to HTMLElement[]
                    ...
                    console.warn(`[WARN] Safe click attempt failed on "${elementInfo.text.substring(0, 30)}..." on ${page.url()}: ${error.message}`);
                }
            }
    ...
    async saveResults() {
        ...
        await fs_1.promises.writeFile(path_1.default.join(outputDir, 'crawl_graph.json'), JSON.stringify(this.graph, null, 2));
        ...
        await fs_1.promises.writeFile(path_1.default.join(outputDir, 'crawl_data.csv'), [csvHeader, ...csvRows].join('\n'));
        ...
        const dotEdges = this.graph.edges.map((edge) => {
            ...
            if (!sourceNode || !targetNode)
                return ''; // Should not happen with normalized URLs
        ...
        await fs_1.promises.writeFile(path_1.default.join(outputDir, 'crawl_graph.dot'), dot);
...
const main = async () => {
    ...
    try {
        ...
        console.error(`[CRITICAL_ERROR] Crawler encountered a critical error: ${error.message}`);
        process.exit(1);
    }
    finally {
...
main().catch(error => {
    console.error(`[UNHANDLED_ERROR] An unhandled exception occurred: ${error.message}`);
    process.exit(1);
});
